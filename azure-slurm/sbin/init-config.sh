...
ACCOUNTING_TAG_NAME=ClusterId
ACCOUNTING_TAG_VALUE=unused
URL=
...
usage() {
    ...
    echo "  --accounting-tag-value <VALUE>     The tag value for accounting (optional). Typically the ClusterId tag generated by CycleCloud."
    echo "  --accounting-tag-name ClusterId    The tag name for accounting (optional). Typically the ClusterId tag generated by CycleCloud."
}
...
while (( "$#" )); do
    case "$1" in
        ...
        --accounting-subscription-id)
            ...
            shift 2
            ;;
        --accounting-tag-value)
            ACCOUNTING_TAG_VALUE=$2
            shift 2
            ;;
        --help)
            usage
...
if [ -z "$CLUSTER_NAME" ] || [ -z "$USERNAME" ] || [ -z "$PASSWORD" ] || [ -z "$URL" ]; then
    ...
    echo "$CLUSTER_NAME"=cluster "$USERNAME"=user  "$PASSWORD"=pass "$URL"=url
    usage
    exit 1
fi
