...
install_pkg=$(jetpack config slurm.install_pkg azure-slurm-install-pkg-4.0.10.tar.gz)
...
if [ $do_install == "True" ]; then
    ...
    python3 install.py --mode $mode --bootstrap-config /opt/cycle/jetpack/config/node.json
fi
