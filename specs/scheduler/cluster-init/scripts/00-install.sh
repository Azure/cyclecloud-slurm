...
if [ $do_install == "True" ]; then
    ...
    python3 install.py --platform $platform --mode scheduler --bootstrap-config $CYCLECLOUD_HOME/config/node.json --properties $CYCLECLOUD_HOME/config/properties.json
    cd ..
fi
