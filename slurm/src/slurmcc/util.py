...
def run(args: list, stdout=subprocesslib.PIPE, stderr=subprocesslib.PIPE, timeout=120, shell=False, universal_newlines=True, **kwargs):
    """
    ...
    """
    try:
        output = subprocesslib.run(args=args, stdout=stdout, stderr=stderr, timeout=timeout, shell=shell, universal_newlines=universal_newlines, **kwargs)
    except subprocesslib.CalledProcessError as e:
